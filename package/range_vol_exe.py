...
data_index=PX.Mul_Data("日收盤表排行","D",'20200916',colist="股票代號",ps="<CM特殊,2112>").iloc[:,0].values
# data=pd.read_excel(r'C:\Users\F128537908\Desktop\data.xlsx')
# data_index=data['標的'].astype(str).values
#stock_set=set(data.iloc[:,0].astype(str).values)
...
warrant_data=warrant_data.drop(2614,axis=0)
#資料前處理
...
final=final[np.append(final.columns[4:].values,final.columns[0:4].values)]

final['檢核']=final[['askvol','avg']].apply(lambda x: "通過" if x['askvol']>x['avg'] else "未通過",axis=1)
del price, S, K, t, r, flag,ratio,index,s_check,warrant_data,vol_for_each,temp
#%%excel
final.to_excel('.\\格式用\\range_vol.xlsx')
count=final.index.levels[0].map(count).values
count=count[:-1]
csum=count.cumsum()
import xlwings as xw 
app = xw.App(visible=True, add_book=False)
wb2 = app.books.open('.\\格式用\\range_vol.xlsx')
wb3 = app.books.open(r'.\\備份\\權證發行分析'+today_str+'.xlsx')
ws2=wb2.sheets[0]
ws2.api.Copy(Before=wb3.sheets[-1].api)
wb3.sheets[-1].delete()
wb3.sheets[-1].name='發行標的檢核'
