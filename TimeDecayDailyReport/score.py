...
#計算剩餘16個交易日 為幾天日曆日
交易日表=pd.read_html("http://172.16.10.13/hedge/tradingday.php")[4]
...
res=requests.get('http://172.16.10.11/hedge/makerauthlist.php')
res.encoding= 'big5'
soup = BeautifulSoup(res.text, 'html.parser')
num=len(soup.find_all(['tr','td'],{"align": "center"}))-1
df={}
temp=[]
for i in range(2,num+2):
    if i % 8 ==1:
        df.update({soup.find_all(['tr','td'],{"align": "center"})[i-8].get_text().strip(): temp })
        temp=[]
    else:
        temp.append(soup.find_all(['tr','td'],{"align": "center"})[i].get_text().strip())
Trader_Table=pd.read_html('http://172.16.10.13/hedge/makerauthlist.php',encoding='utf-8')[0]        
x=pd.DataFrame(df).T
x.columns=Trader_Table.columns[1:]
Trader_Table=x.copy()

Trader_Table['群組代碼']=Trader_Table['群組代碼'].astype(str)
Trader_Table=Trader_Table.reset_index()
Trader_Table=Trader_Table.rename(index={'index':'權證代碼'})
...
if last_work_day_str in datelist:
    ...
    for scoretype in ['價格穩定分數','價差大小分數','報價數量分數']:
        temp=BrokerScore_Table.reindex(brokerlist)[scoretype].to_list()
        #temp=BrokerScore_Table.loc[BrokerScore_Table.index.intersection(brokerlist),scoretype].to_list()
...
if row_max>=15:#要有14日平均
    ...
    Trader_Period=Trader_Period.dropna()
    print("交易員移動平均完成")
...
if last_work_day_str in datelist:#日期不重複
    ...
    for scoretype in ['價格穩定分數','價差大小分數','報價數量分數']:
        temp=TraderScore_Table_final.reindex(traderlist).loc[:,scoretype].to_list() 
        temp.insert(0,last_work_day_str)
